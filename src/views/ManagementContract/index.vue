    ...
    >
      ...
      <el-table-column min-width="130%" align="center" :label="$t('contract.operation')">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.contractStatus === '1'" type="primary" size="mini" @click="handlePay()">
            查看订单
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    <contract-dialog v-if="contractComplete" :title="contract.title" :image="contractComplete" :remote-close="remoteClose" :visible="contract.visible" />
  </div>
</template>
