	...
	<div class="box">
		...
		<div class="block">
			...
			<div class="rightbutton">
				<el-button @click="switchTo('sharedfile')" type="primary">返回共享文件</el-button>
			</div>
		</div>
		<!-- 表格 -->
		<el-table :data="resp" style="width: 100%" border>
			...
			<el-table-column label="操 作" align="center">
				<template slot-scope="scope">
					...
					<el-button size="mini" type="warning" @click="deletecollectfiles(scope.row.id)"
					>取消收藏</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
import { queryAlCollectionlFiles,cancelCollection,queryPaging } from '~/api/sharedFile';
// 封装的事件处理工具
	...
	methods: {
		...
		deletecollectfiles(id) {
			this.$confirm('您确认收藏这个文件吗？', '提示', {
				...
				cancelCollection({fileId:id,userId:this.userInfo.uid}).then(response => {
		...
		searchfiles(fileName) {
			queryPaging({ current: this.current, size: this.size,fileName:this.fileName}).then((response) => {
		...
		// 根据label标签搜索
		searchlabels(select) {
				queryPaging({ current:this.current,size: this.size,select:this.select}).then((response) => {
				console.log(select);
				const data = response.data.fileIPage;
				//  总条数
				this.total = data.total;
